...
Well, this tool uses IndexedDB to store large amounts of files, and in a fast database system. It also uses ServiceWorker to intercept requests on the same domain (so `/n/myFolder/index.html` isn't an actual URL, but merely a simulated generated by the ServiceWorker). It also intercepts requests from the JavaScript and uses some small regexes in order to handle edge cases, allowing the ServiceWorker to pick up on those links too!
