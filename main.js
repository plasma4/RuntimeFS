  ...
  async function process(src, dst) {
    for await (const entry of src.values()) {
      ...
      if (entry.kind === "file") {
        ...
        const newDst = await dst.getDirectoryHandle(entry.name, {
          create: true,
        });
...
async function createDownloadStream(filename) {
  if (window.showSaveFilePicker) {
    try {
      ...
      console.warn(
        "Native File System API failed, falling back to memory buffer:",
        err
      );
    }
  }
...
async function exportData() {
  ...
  try {
    const fileWritable = await createDownloadStream(
      encrypt ? "result.enc" : "result.tar.gz"
    );
    let outputStream;
    ...
    const metadata = {
      ...
      ss: document.getElementById("c7").checked
        ? getSessionStorageExport()
        : {},
      cookies:
        typeof getCookiesAsObject === "function" &&
    ...
    await writeTarEntry(
      "runtimefs_system/metadata.json",
      new TextEncoder().encode(JSON.stringify(metadata, null, 2))
    );
    if (document.getElementById("c3").checked)
      await streamIndexedDBToWriter(updateProgress, writeTarEntry);
    if (document.getElementById("c6").checked)
      await streamCacheStorageToWriter(updateProgress, writeTarEntry);
    ...
    try {
      ...
      for (const storeName of storeNames) {
        ...
        while (hasMore) {
          ...
          const { batch, nextKey, completed } = await new Promise(
            (resolve, reject) => {
              const tx = db.transaction(storeName, "readonly");
              const store = tx.objectStore(storeName);
              // Resume from lastKey if we have one
              const range =
                lastKey !== null ? IDBKeyRange.lowerBound(lastKey, true) : null;
          ...
          if (batch.length > 0) {
            ...
            await writeFunc(
              `__IDB_DATA__/${dbName}/${storeName}/${chunkId++}`,
              chunkData
            );
          }
...
async function startImport(file) {
  ...
  if (navigator.serviceWorker.controller)
    navigator.serviceWorker.controller.postMessage({
      type: "PREPARE_FOR_IMPORT",
    });

  const restoreFromCbor = (item) => {
    if (!item || typeof item !== "object") return item;
    if (item.__rfs_blob && item.data)
  ...
  try {
    ...
    while (true) {
      ...
      const isSystemFile =
        path === "runtimefs_system/metadata.json" ||
      ...
      if (isSystemFile) {
        ...
        if (path === "runtimefs_system/metadata.json") {
          try {
            ...
            console.warn("Metadata corruption!", e);
          }
          ...
          try {
            await new Promise((r) => {
              const q = indexedDB.deleteDatabase(schema.dbName);
              q.onsuccess = r;
              q.onerror = r;
            });
            ...
            console.warn("Schema restore failed", e);
          }
          ...
          try {
            ...
            console.warn("IDB Data write failed", e);
          }
        }
